...
export function statusOf(c){
  ...
  if (c.claim.cancelled)      return { key:"rejected",  label:"rejected",   kind:"bad"  };
  if (c.claim.committed)      return { key:"committed", label:"committed",  kind:"info" };
  if (c.claim.issuerApproved) return { key:"approved",  label:"approved",   kind:"info" };
...
export function RebindProvider({ children }){
  ...
  const goTo = useCallback((id) => setBeat((b) => {
    const i = beatsRef.current.indexOf(id);
    return i >= 0 ? i : b;
  }), []);

  const atOrPast = useCallback((id) => beat >= beats.indexOf(id), [beat, beats]);
  ...
  const refreshClaims = useCallback(async (count) => {
  ...
  const refreshAdvance = useCallback(async (id = claimId) => {
    if (!advance.enabled || id === null || id === undefined){ setAdv(null); return null; }
   ...
   */
  const resume = useCallback(async (cfg) => {
    const c = cfg || config;
    if (!c) return false;
    const w = c.wallets || {};
    ...
    if (st.claimCount > 0){
      ...
      if (cl){
        ...
        if (cl.claim.cancelled){ setRejected(true); jump("review"); return true; }
  ...
  /* Connection state lives in its own listener set inside api.js, so it needs
     ...
     backend goes away. */
  useEffect(() => onConnectionChange((c) => {
    setOnline(c.online);
  ...
  useEffect(() => {
    ...
    const t = setInterval(() => { if (!document.hidden) refreshState(); }, 4000);
  ...
  const value = useMemo(() => ({
    ...
    liveGuardian, activeGuardianRequest,
    recoverSubMode, setRecoverSubMode,
    refreshState, refreshClaims, refreshAdvance,
    // connection
    online, connDetail,
    // demo flow
    beats, beat, beatId, setBeat, goTo, atOrPast,
    claimId, setClaimId,
