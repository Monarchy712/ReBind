...
export default function Recover(){
  ...
  const [gForm, setGForm] = useState({ customerId: "", wallet: "", newGuardian: "" });
  ...
  const handleOpenGuardianRequest = async () => {
    if (!gForm.customerId.trim() || !gForm.wallet.trim() || !gForm.newGuardian.trim()) {
      toast("All fields are required.", { title: "Missing details", kind: "warn" });
      return;
    }
    try {
      const st = await safe.state([gForm.wallet.trim()], gForm.customerId.trim());
      const oldGuardian = st?.liveGuardian;
      if (!oldGuardian || oldGuardian === "0x0000000000000000000000000000000000000000") {
        throw new Error("Could not locate active guardian for this identity. Is the wallet bound?");
      }
      if (oldGuardian.toLowerCase() === gForm.newGuardian.trim().toLowerCase()) {
        throw new Error("New guardian must be different from the old guardian.");
      }
      ...
      toast(`Guardian replacement request opened!`, { title: "Request opened", kind: "ok" });
      await S.refreshState();
    } catch (e) {
      toast(e.message, { title: "Could not open request", kind: "err", ms: 8000 });
    }
  };
    ...
    return (
      <>
        ...
        <p style={{ color: "var(--muted)", maxWidth: "62ch", lineHeight: 1.6, marginBottom: 24 }}>
          A request to change the recovery guardian has been opened. It is currently in the challenge window to allow objections.
          Once the window has elapsed, the change can be finalized on-chain.
        </p>

        <Ring remaining={req.timeRemaining} total={S.cureWindow} label="time until change takes effect" />

        <div className="stats" style={{ marginBottom: 22, marginTop: 22 }}>
          <div className="stat"><div className="k">Wallet</div><div className="v" style={{ fontSize: 14, fontFamily: "var(--mono)" }}>{short(req.wallet)}</div></div>
          <div className="stat"><div className="k">Old Guardian</div><div className="v" style={{ fontSize: 14, fontFamily: "var(--mono)" }}>{short(req.oldGuardian)}</div></div>
          <div className="stat"><div className="k">New Guardian</div><div className="v" style={{ fontSize: 14, fontFamily: "var(--mono)" }}>{short(req.newGuardian)}</div></div>
        </div>
            ...
            <ActionButton
              ...
              onAction={async () => {
                try {
                  ...
                  toast("Guardian replaced successfully!", { title: "Success", kind: "ok" });
                  await S.refreshState();
                } catch (e) {
  ...
  const renderClaimBlocked = () => {
    ...
    return (
      <>
        <h2 className="display" style={{ fontSize: "var(--step-3)", marginBottom: 14 }}>
          Recovery Blocked
        </h2>
        <div className="banner warn" style={{ marginBottom: 22 }}>
          ...
          <div>
            ...
            <p>
              A guardian replacement request is currently active for this identity.
              You cannot open a new recovery claim until the guardian change resolves or is cancelled.
            </p>
          </div>
        </div>
    ...
    return (
      <>
        ...
        <p style={{ color: "var(--muted)", maxWidth: "60ch", lineHeight: 1.6, marginBottom: 24 }}>
          If you have lost access to your guardian, you can request a replacement.
          This starts a public cure window during which the issuer or the current guardian can object.
        </p>
        <div className="field">
        ...
        <div className="field">
          ...
          <input
            ...
            onChange={(e) => setGForm({ ...gForm, newGuardian: e.target.value })}
            spellCheck="false"
            autoComplete="off"
          />
          <div className="help">
            The new guardian will co-sign any future recovery claims for this identity.
          </div>
        </div>
        <div className="row" style={{ gap: 10 }}>
          <button className="btn" onClick={() => setSubMode("claim")}>
          ...
          <ActionButton className="btn primary" onAction={handleOpenGuardianRequest}>
  ...
  const body = (() => {
    if (S.activeGuardianRequest && !S.activeGuardianRequest.cancelled && !S.activeGuardianRequest.finalized) {
    ...
    if (key === "identity") return (
      <>
        ...
        <div className="row" style={{ gap: 10, alignItems: "center" }}>
          ...
          <a href="#" onClick={(e) => { e.preventDefault(); setSubMode("guardian"); }} style={{ fontSize: 14, color: "var(--muted)", textDecoration: "underline" }}>I've lost access to my guardian too</a>
        </div>
      </>
    );
  ...
  return (
    <div className="view page">
      <div className="wrap">
        ...
        <div className={subMode === "claim" && !S.activeGuardianRequest ? "wizard" : "wizard-no-steps"}>
