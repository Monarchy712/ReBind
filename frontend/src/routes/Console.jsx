...
export default function Console(){
  ...
  const guardianRequests = (guardianRequestsRaw || []).map((r) => ({
    ...r,
    timeRemaining: r.cancelled || r.finalized
      ? r.timeRemaining
      ...
      <div className="wrap">
          ...
          <Reveal className="panel pad" delay={100} style={{ marginTop: 24 }}>
            ...
            <div style={{ marginTop: 14 }}>
              <p style={{ color: "var(--muted)", fontSize: 14, marginBottom: 14 }}>
                The fallback receiver receives repayments if the main vault address is blocked or revoked on the registry.
                It must be bound to an identity in the registry to receive NOTE transfers.
              </p>
              <div className="row" style={{ gap: 14, alignItems: "flex-end" }}>
                ...
                <ActionButton
                  ...
                  onAction={async () => {
                    ...
                    try {
                      await api.vault.setFallbackReceiver(newFallbackAddress.trim());
                      toast("Fallback receiver updated successfully.", { title: "Success", kind: "ok" });
                      await load();
                    } catch (e) {
        ...
        {S.advance.enabled && (
          <Reveal className="panel" delay={120} style={{ marginTop: 24 }}>
            <div className="panel-head">
              ...
              <span className="chip bad">{repaymentFailures.length} failed</span>
            </div>
            <div className="tablewrap">
              <table className="data">
                <thead>
                  ...
                  {repaymentFailures.length === 0 ? (
                    <tr>
                      <td colSpan={4}>
                        <div className="empty" style={{ padding: "20px 0" }}>
                          <p style={{ margin: 0 }}>No failed repayments recorded.</p>
                        </div>
                      </td>
                    </tr>
                  ) : (
                    repaymentFailures.map((fail, i) => (
                      <tr key={i}>
                        ...
                        <td className="addr" title={fail.receiver}>{short(fail.receiver)}</td>
        ...
        <Reveal className="panel" delay={140} style={{ marginTop: 24 }}>
          <div className="panel-head">
            ...
            <span className="chip info">{guardianRequests.filter(r => !r.cancelled && !r.finalized).length} active</span>
          </div>
          <div className="tablewrap">
            <table className="data">
              <thead>
              ...
              <tbody>
                {guardianRequests.length === 0 ? (
                  <tr>
                    <td colSpan={7}>
                      <div className="empty" style={{ padding: "20px 0" }}>
                        <p style={{ margin: 0 }}>No guardian replacement requests found.</p>
                      </div>
                    </td>
                  </tr>
                ) : (
                  guardianRequests.map((req) => {
                    ...
                    return (
                      <tr key={req.requestId}>
                        ...
                        <td className="addr" title={req.wallet}>{short(req.wallet)}</td>
                        <td className="addr" title={req.oldGuardian}>{short(req.oldGuardian)}</td>
                        <td className="addr" title={req.newGuardian}>{short(req.newGuardian)}</td>
                        <td>
                          {req.cancelled ? (
                        ...
                        <td className="mono tnum">
                          {done ? "—" : elapsed ? "elapsed" : clock(req.timeRemaining)}
                        </td>
                        <td>
                          <div className="rowacts" style={{ justifyContent: "flex-end" }}>
                            {!done ? (
                              <ActionButton
                                ...
                                onAction={async () => {
                                  try {
                                    await api.guardian.cancelRequest(req.requestId);
                                    toast(`Request #${req.requestId} objected/cancelled.`, { title: "Cancelled", kind: "ok" });
                                    await load();
                                  } catch (e) {
                                    toast(e.message, { title: "Object failed", kind: "err" });
                                  }
                                }}
                              >
                                Object
